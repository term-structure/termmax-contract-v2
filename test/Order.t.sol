...
contract OrderTest is Test {
    ...
    function testSwapCoverMultiCuts(bool isBuy, bool isFt, uint128 swapAmt) public {
        ...
        if (isBuy) {
            tokenIn = res.debt;
            tokenOut = isFt ? res.ft : res.xt;
        } else {
            tokenIn = isFt ? res.ft : res.xt;
            tokenOut = res.debt;
        }

        OrderConfig memory specialConfig = JSONLoader.getOrderConfigFromJson(testdata, ".specOrderConfig");
        vm.startPrank(maker);
        ITermMaxOrder order = res.market.createOrder(
            maker, specialConfig.maxXtReserve, ISwapCallback(address(0)), specialConfig.curveCuts
        );
        res.debt.mint(maker, maxSwapAmt);
        res.debt.approve(address(res.market), maxSwapAmt);
        res.market.mint(address(order), maxSwapAmt);
        vm.stopPrank();

        vm.startPrank(sender);
        res.debt.mint(sender, maxSwapAmt * 2);
        res.debt.approve(address(res.market), maxSwapAmt);
    ...
    function testSwapCoverMultiCuts2(bool isBuy, bool isFt, uint128 swapAmt) public {
        uint256 maxSwapAmt = 39_200_000_000;
        vm.assume(swapAmt > 0 && swapAmt < maxSwapAmt);
        if (isBuy && !isFt) {
            vm.assume(swapAmt <= maxSwapAmt / 1000);
        }
        ...
        if (isBuy) {
            ...
            uint128 debtTokenOutAmt;
            if (isFt) {
                debtTokenOutAmt = swapAmt / 2;
            } else {
                debtTokenOutAmt = swapAmt / 1000;
            }
            order.swapTokenToExactToken(tokenIn, tokenOut, sender, debtTokenOutAmt, swapAmt, block.timestamp + 1 hours);
        }
        vm.stopPrank();
    }
