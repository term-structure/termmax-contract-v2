...
library LoanUtils {
    ...
    function calcLiquidationResult(
        ...
        uint256 repayAmt
    ) internal view returns (uint cToLiquidator, uint cToTreasurer, uint remainningC) {
        ...
        uint cEqualRepayAmt = (repayAmt * udPriceToCdPrice * cDecimals) / (uDecimals * Constants.DECIMAL_BASE);

        uint rewardToLiquidator = (cEqualRepayAmt * REWARD_TO_LIQUIDATOR) / Constants.DECIMAL_BASE;
        uint rewardToProtocol = (cEqualRepayAmt * REWARD_TO_PROTOCOL) / Constants.DECIMAL_BASE;
