...
import {ITermMaxTokenPair, TermMaxTokenPair} from "../../contracts/core/TermMaxTokenPair.sol";
...
library DeployUtils {
    ...
    function deployMarket(
        address deployer,
        TokenPairConfig memory tokenPairConfig,
        ...
        ITermMaxFactory.TokenPairDeployParams memory tokenPairParams = ITermMaxFactory
            .TokenPairDeployParams({
                gtKey: GT_ERC20,
                admin: deployer,
        ...
        res.tokenPairConfig = tokenPairConfig;
        res.tokenPair = ITermMaxTokenPair(res.factory.createTokenPair(tokenPairParams));

        ITermMaxFactory.MarketDeployParams memory marketParams = ITermMaxFactory
            .MarketDeployParams({
                admin: deployer,
        ...
        res.market = ITermMaxMarket(res.factory.createMarket(marketParams));
    ...
    function deployMarket(
        ...
        ITermMaxFactory.MarketDeployParams memory params = ITermMaxFactory
            .MarketDeployParams({
                admin: deployer,
                tokenPair: res.tokenPair,
                marketConfig: marketConfig
    ...
    function deployFactory(address admin) internal returns (TermMaxFactory factory) {
        ...
        TermMaxTokenPair tokenPair = new TermMaxTokenPair();
        factory.initTokenPairImplement(address(tokenPair));
        TermMaxMarket market = new TermMaxMarket();
        factory.initMarketImplement(address(market));
    }
