...
contract DeployBaseV2 is Script {
    ...
    function deployOrUpgradeRouter(address admin, address routerProxy) public returns (TermMaxRouterV2 router) {
        ...
        if (routerProxy != address(0)) {
            ...
            require(existingRouter.getImplementation() != address(implementation), "Already upgraded");
            existingRouter.upgradeTo(address(implementation));
        ...
        returns (AccessManagerV2 accessManager)
    {
        ...
        if (accessmanagerProxy != address(0)) {
            ...
            AccessManager existingAccessManager = AccessManager(accessmanagerProxy);
            require(existingAccessManager.getImplementation() != address(implementation), "Already upgraded");
            existingAccessManager.upgradeTo(address(implementation));
