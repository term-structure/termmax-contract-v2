 ...
 *
 * Usage:
 *   forge script script/deploy/DeployPTReUSDeAdapter.s.sol:DeployPTReUSDeAdapter \
 *     --rpc-url $ETH_MAINNET_RPC_URL \
 *     --private-key $ETH_MAINNET_DEPLOYER_PRIVATE_KEY \
 *     --broadcast \
 *     --verify
 */
contract DeployPTReUSDeAdapter is DeployBaseV2 {
    ...
    function setUp() public {
        ...
        console.log("Spark PTReUSDe Linear Oracle:", SPARK_PTREUSDE_LINEAR_ORACLE);
    }
    ...
    function run() public {
        ...
        TermMaxSparkLinearOracleAdapter adapter = new TermMaxSparkLinearOracleAdapter(SPARK_PTREUSDE_LINEAR_ORACLE);

        vm.stopBroadcast();

        console.log("\n===== Deployment Successful =====");
        console.log("TermMaxSparkLinearOracleAdapter deployed at:", address(adapter));
        console.log("Wrapping Spark Linear Oracle at:", SPARK_PTREUSDE_LINEAR_ORACLE);
    ...
    function saveDeploymentInfo(address adapterAddress) internal {
        ...
        string memory json = string(
            abi.encodePacked(
                ...
                '",\n',
                '    "sparkPtReUSDeLinearOracle": "',
                vm.toString(SPARK_PTREUSDE_LINEAR_ORACLE),
                '"\n',
                "  }\n",
                "}"
            )
