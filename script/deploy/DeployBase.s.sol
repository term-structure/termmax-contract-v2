...
import {MarketInitialParams, LoanConfig} from "../../contracts/storage/TermMaxStorage.sol";
...
contract DeployBase is Script {
    ...
    function deployCore(
        ...
        public
        returns (ITermMaxFactory factory, IOracle oracle, ITermMaxRouter router, SwapAdapter swapAdapter, Faucet faucet)
    {
        // deploy factory
        factory = deployFactory(adminAddr);

        // deploy oracle aggregator
        oracle = deployOracle(adminAddr);
    ...
    // ) public returns (ITermMaxMarket[] memory markets) {
    ...
    //     markets = new ITermMaxMarket[](configs.length);
    ...
    //             tokenName: config.marketConfig.marketName,
    //             tokenSymbol: config.marketConfig.marketSymbol
    //         });

    //         ITermMaxMarket market = ITermMaxMarket(factory.createMarket(GT_ERC20, initialParams));
    //         // ITermMaxFactory.DeployParams memory params = ITermMaxFactory.DeployParams({
    //         //     gtKey: keccak256(bytes(config.collateralConfig.gtKeyIdentifier)),
    //         //     admin: adminAddr,
    //         //     collateral: address(collateral),
    //         //     underlying: IERC20Metadata(address(underlying)),
    //         //     oracle: IOracle(address(underlyingPriceFeed)),
    //         //     liquidationLtv: config.marketConfig.liquidationLtv,
    //         //     maxLtv: config.marketConfig.maxLtv,
    //         //     liquidatable: config.marketConfig.liquidatable,
    //         //     marketConfig: marketConfig,
