...
contract DeployPriceFeedFactory is DeployBaseV2 {
    ...
    function run() public {
        ...
        if (coreContracts.priceFeedFactory == TermMaxPriceFeedFactoryV2(address(0))) {
            coreContracts.priceFeedFactory = new TermMaxPriceFeedFactoryV2();
            if (isBroadcast) {
                writeAsJson(
                    string.concat(
                        vm.projectRoot(), "/deployments/", coreParams.network, "/", coreParams.network, "-core-v2.json"
            ...
            console.log("Deployed TermMaxPriceFeedFactoryV2 at:", address(coreContracts.priceFeedFactory));
        } else {
            revert("PriceFeedFactory already deployed");
        }
