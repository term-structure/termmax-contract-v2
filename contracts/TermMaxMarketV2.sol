...
import {StakingBuffer} from "./extensions/StakingBuffer.sol";
/**
    ...
    MarketEvents
{
    ...
    function _mint(address caller, address recipient, uint256 debtTokenAmt) internal {
        ...
        _depositWithBuffer(address(debtToken), debtTokenAmt);
    ...
    function _burn(address owner, address spender, address recipient, uint256 debtTokenAmt) internal {
        ...
        _withdrawWithBuffer(address(debtToken), recipient, debtTokenAmt);
    ...
    function _leverageByXt(
        ...
        _withdrawWithBuffer(address(debtToken), loanReceiver, xtAmt);
        // Callback function
        ...
        returns (uint256 debtTokenAmt, bytes memory deliveryData)
    {
        ...
        debtTokenAmt = debtToken.balanceOf(address(this)) + aToken.balanceOf(address(this));

        debtTokenAmt = (debtTokenAmt * proportion) / Constants.DECIMAL_BASE_SQ;
    }
    ...
    function _withdrawFromPool(address assetAddr, address to, uint256 amount) internal virtual override {
        aavePool.withdraw(assetAddr, amount, to);
    }
}
