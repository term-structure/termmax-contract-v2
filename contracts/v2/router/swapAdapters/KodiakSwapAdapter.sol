...
import {IKodiakRouter} from "../../extensions/kodiak/IKodiakRouter.sol";
...
contract KodiakSwapAdapter is ERC20SwapAdapterV2 {
        ...
        returns (uint256 tokenOutAmt)
    {
        ...
        input.amount = amount;
        output.receiver = recipient;
        uint256 outputAmountBefore = tokenOut.balanceOf(recipient);
        tokenIn.safeApprove(address(router), amount);
        router.swap(input, output, swapDataStruct, feeData);
        tokenOutAmt = tokenOut.balanceOf(recipient) - outputAmountBefore;
    }
}
