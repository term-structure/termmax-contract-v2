...
contract PriceFeedConverter is AggregatorV3Interface {
    ...
    constructor(address _aTokenToBTokenPriceFeed, address _bTokenToCTokenPriceFeed) {
        ...
        priceDemonitor = int256(10 ** aTokenToBTokenPriceFeed.decimals()) * int256(10 ** bTokenToCTokenPriceFeed.decimals());
    }
    ...
    function description() external view returns (string memory) {
        return string(abi.encodePacked(aTokenToBTokenPriceFeed.description(), " - ", bTokenToCTokenPriceFeed.description()));
    }
    ...
    function latestRoundData() external view returns (uint80, int256, uint256, uint256, uint80) {
        (uint80 roundId, int256 answer, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound) = aTokenToBTokenPriceFeed.latestRoundData();
